...
import { IExpensesState, initialState } from './initial-state';
import { ExpensesContainer, CheckIcon } from './style';
...
import { getAllExpenses } from '../../services/domain/expenses/get-all';
import { RiMoneyCnyBoxFill } from 'react-icons/ri';
import SelectRecurence from '../../../../components/finax/SelectRecurence';
import { createExpenses } from '../../services/domain/expenses/create';
import { deleteExpenses } from '../../services/domain/expenses/delete';
import { updateExpenses } from '../../services/domain/expenses/update';
...
const Expenses: React.FC = () => {
  ...
  const { initialExpensetState } = initialState();

  const [expenseState, setexpenseState] = useState<IExpensesState>(initialExpensetState);
  ...
  const validateForm = () : boolean => {
    let isOk = true;
    const { name, due_date, recurrence, value } = expenseState;
    // if(!expense_type_id){
  ...
  const handleSave = async () => {
    console.log("vexpenseState", expenseState);
    if(validateForm()){
      createExpenses({...expenseState, value: parseFloat(expenseState.value.replace('.','').replace(',', '.')) })
      .then(() => {
  ...
  const handleUpdate = () => {
    if(validateForm()){
      updateExpenses({...expenseState, value: parseFloat(expenseState.value.replace('.','').replace(',', '.'))})
      .then(() => {
  ...
  const handleDelete = (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteExpenses({ id }).then((response)=>{
  ...
  const handleChangeValues = (id: string) => (event: ChangeEvent<{ value: any; name: string; checked?: boolean }>, newValue: any = '') => {
    ...
    setexpenseState((prevState: any): IExpensesState => {
      ...
      newState[id] = value;
      return { ...newState } as IExpensesState;
    });
  };
  ...
  const disableCreateButton = (): boolean => {
    if(mode === 'edit') return false;
     ...
     const { name, due_date,  recurrence, value } = expenseState;
     return !(!!name && !!due_date &&  !!recurrence && !! value);
  };

  const resetStates = () => {
    setexpenseState(initialExpensetState);
  };
    ...
    <ExpensesContainer>
      <Grid container>
        <Grid item xs={12}>
          ...
          <Grid container className="mb-16">
            <Grid item xs={12}>
              <TitlePages section="Empresa" title="Despesas fixas" showAction open={showFilter} handleOpen={() => setShowFilter(!showFilter)} icon={<MdStorefront size={29}/>} />
            </Grid>
          </Grid>
                  ...
                  <Grid item xs={6} sm={4} lg={2} xl={1}>
                    <Box justifyContent="flex-end" display="flex" className="btn-action-new-filter">
                      ...
                      >
                        ...
                        <div className="text">Nova despesa</div>
                      </button>
                    </Box>
                  </Grid>
              </Grid>
              ...
              <Grid container direction="column" spacing={1}>
                ...
                <Grid item>
                  <SelectRecurence
                    ...
                    label="Recorrência"
                    disable={mode === 'view'}
                    value={{ code: expenseState.recurrence }}
                ...
                <Grid item className="p-8">
                    <FormControlLabel
                      value="top"
                      control={<Switch disabled={mode === 'view'} color="primary" name='status' onChange={handleChangeValues('status')} checked={expenseState.status === 'active'} />}
                      label="Status"
                    />
                </Grid>
              </Grid>
